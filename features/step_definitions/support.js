...
Support.prototype.get = function(sut, url, callback){
  sut.browser.get(url).then(function(result) {
    sut.browser.refresh();   // Hack for strange browser startup behavior...
    callback(result)
  });
};
...
Support.prototype.findById = function(sut, item, callback){
    sut.browser.findElement({id : item}).then(function(result) {
    callback(result);
  });
};
